...
local function setrank(msg, name, value) -- setrank function
  ...
  if hash then
	...
	return send_msg('chat#id'..msg.to.id, 'مقام ثبت شد', ok_cb,  true)
  end
end
--delrank----------------------------------------------------------------------------------------------------
local function delrank(msg, name) -- delrank function
  local hash = nil
  if msg.to.type == 'chat' then
    hash = 'rank:'..msg.to.id..':variables'
  end
  if hash then
	...
	return send_msg('chat#id'..msg.to.id, 'از مقام خود عزل شد', ok_cb,  true)
  end
end
...
--setrank by reply -------------------------------------------------------------------------------------
local function setrank_by_reply(extra, success, result)
local value = extra.value
setrank(result, result.from.id, value)
end
--delrank by reply -------------------------------------------------------------------------------------
local function delrank_by_reply(extra, success, result)
setrank(result, result.from.id)
end
--run ---------------------------------------------------------------------------------------------------
...
if matches[1] == '/infodel' and is_sudo(msg) then
  ...
  if msg.reply_id then
    msgr = get_message(msg.reply_id, delrank_by_reply, {receiver=receiver, Reply=Reply, value=value})
  else
  local name = string.sub(matches[2], 1, 50)
  local text = delrank(msg, name)
  return text
  end
end
if matches[1]:lower() == '/info' and is_sudo(msg) then
  local hash = 'usecommands:'..msg.from.id..':'..msg.to.id
  redis:incr(hash)
  local receiver = get_receiver(msg)
  local Reply = msg.reply_id
  ...
  local value = string.sub(matches[2], 1, 1000)
    msgr = get_message(msg.reply_id, setrank_by_reply, {receiver=receiver, Reply=Reply, value=value})
  else
  local name = string.sub(matches[2], 1, 50)
...
local usertype = (redis:hget(hash, user) or "-----")
--hardware ------------------------------------------------------------------------------------------------
